...
#include "ExternalSource/urdf/model.h"
#include "ExternalSource/urdf/urdf_parser.h"
...
namespace RigidBodyDynamics {
  namespace Addons {
    ...
    bool construct_model (Model* rbdl_model, ModelPtr urdf_model, bool verbose) {
      boost::shared_ptr<dynacore::urdf::Link> urdf_root_link;
      ...
      // add the root body
      const boost::shared_ptr<const dynacore::urdf::Link>& root = urdf_model->getRoot ();
      ...
      for (j = 0; j < joint_names.size(); j++) {
        ...
        if (0 && urdf_joint->type == dynacore::urdf::Joint::FLOATING) {
    ...
    RBDL_DLLAPI bool URDFReadFromString (const char* model_xml_string, Model* model, bool verbose) {
      ...
      boost::shared_ptr<dynacore::urdf::ModelInterface> urdf_model = dynacore::urdf::parseURDF (model_xml_string);
