...
#include <Atlas_Controller/CtrlSet/TransitionConfigCtrl.hpp>
...
#include <Atlas_Controller/CtrlSet/JPosTrajPlanningCtrl.hpp>
    ...
    num_step_(0)
{
    ...
    right_swing_start_trans_ctrl_ = 
        new TransitionConfigCtrl(robot, atlas_link::rightFoot, false);
    right_swing_end_trans_ctrl_ = 
        new TransitionConfigCtrl(robot, atlas_link::rightFoot, true);
    // Left
    left_swing_start_trans_ctrl_ = 
        new TransitionConfigCtrl(robot, atlas_link::leftFoot, false);
    left_swing_end_trans_ctrl_ = 
        new TransitionConfigCtrl(robot, atlas_link::leftFoot, true);
    // Swing Controller Selection
    config_right_swing_ctrl_ = 
        new JPosTrajPlanningCtrl(robot_sys_, atlas_link::rightFoot, reversal_planner_);
    config_left_swing_ctrl_ = 
        new JPosTrajPlanningCtrl(robot_sys_, atlas_link::leftFoot, reversal_planner_);
...
void WalkingConfigTest::TestInitialization(){
    // Planner
    reversal_planner_->PlannerInitialization(AtlasConfigPath"PLANNER_velocity_reversal");
    // Yaml file name
...
void WalkingConfigTest::_SettingParameter(){
    // Setting Parameters
    ParamHandler handler(AtlasConfigPath"TEST_walking_config.yaml");
