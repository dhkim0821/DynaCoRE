    ...
    mu_stance_(0.3)
{
  ...
  Jc_ = dynacore::Matrix(6, atlas::num_qdot);
...
bool DoubleContactBounding::_UpdateJc(){
  ...
  Jc_.block(0, 0, 3, atlas::num_qdot) = Jtmp.block(3, 0, 3, atlas::num_qdot);

  robot_sys_->getFullJacobian(atlas_link::leftFoot, Jtmp);
  Jc_.block(3, 0, 3, atlas::num_qdot) = Jtmp.block(3, 0, 3, atlas::num_qdot);

   //dynacore::pretty_print(Jc_, std::cout, "[double contact bounding] Jc");
  return true;
}
bool DoubleContactBounding::_UpdateJcDotQdot(){
  dynacore::Matrix JcDot(dim_contact_, atlas::num_qdot);
  dynacore::Matrix jcdot_tmp;
  // Right
  robot_sys_->getFullJacobianDot(atlas_link::rightFoot, jcdot_tmp);
  JcDot.block(0, 0, 3, atlas::num_qdot) = jcdot_tmp.block(3, 0, 3, atlas::num_qdot);
  // Left
  robot_sys_->getFullJacobianDot(atlas_link::leftFoot, jcdot_tmp);
  JcDot.block(3, 0, 3, atlas::num_qdot) = jcdot_tmp.block(3, 0, 3, atlas::num_qdot);
...
bool DoubleContactBounding::_UpdateUf(){
  ...
  dynacore::Matrix U;
  if(trans_pt_ == atlas_link::rightFoot){
      _setU(mu_trans_, U);
      ...
      Uf_(size_u*2, 2) = -1.;
  }else if(trans_pt_ == atlas_link::leftFoot){
      _setU(mu_stance_, U);
      Uf_.block(0, 0, size_u, 3) = U;
      _setU(mu_trans_, U);
      Uf_.block(size_u, 3, size_u, 3) = U;
