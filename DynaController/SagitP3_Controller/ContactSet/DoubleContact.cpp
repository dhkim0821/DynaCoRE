...
DoubleContact::DoubleContact(const RobotSystem* robot):WBDC_ContactSpec(10)
{
  ...
  Jc_ = dynacore::Matrix(dim_contact_, sagitP3::num_qdot);
...
bool DoubleContact::_UpdateJc(){
  ...
  // Left
  robot_sys_->getOri(sagitP3_link::lAnkle, quat_tmp);
  ...
  robot_sys_->getFullJacobian(sagitP3_link::lAnkle, Jtmp);
  J_local = lAnkle_rot_mt.transpose() * Jtmp;
  Jc_.block(0, 0, 5, sagitP3::num_qdot) = J_local.block(1, 0, 5, sagitP3::num_qdot);

  // Right
  robot_sys_->getOri(sagitP3_link::rAnkle, quat_tmp);
  ...
  robot_sys_->getFullJacobian(sagitP3_link::rAnkle, Jtmp);
  J_local = rAnkle_rot_mt.transpose() * Jtmp;
  Jc_.block(5, 0, 5, sagitP3::num_qdot) = J_local.block(1, 0, 5, sagitP3::num_qdot);
...
bool DoubleContact::_UpdateJcDotQdot(){
    // TODO
  dynacore::Matrix JcDot(dim_contact_, sagitP3::num_qdot);
